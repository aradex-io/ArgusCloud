"""Rate limiting for CloudGraph API.
...
def init_rate_limiting(
    ...
    if storage_uri is None:
        storage_uri = os.environ.get("CLOUDGRAPH_RATELIMIT_STORAGE", "memory://")

    # Get default limit from environment
    default_limit = os.environ.get(
        "CLOUDGRAPH_RATELIMIT_DEFAULT",
...
def get_endpoint_limit(endpoint: str) -> str:
    ...
    return os.environ.get(
        f"CLOUDGRAPH_RATELIMIT_{endpoint.upper()}",
...
class RateLimitConfig:
    ...
    def __init__(
        ...
        for key, value in DEFAULT_LIMITS.items():
            if key not in self.endpoint_limits:
                env_key = f"CLOUDGRAPH_RATELIMIT_{key.upper()}"
    ...
    def from_env(cls) -> "RateLimitConfig":
        ...
        """
        enabled = os.environ.get("CLOUDGRAPH_RATELIMIT_ENABLED", "true").lower() == "true"
