"""API authentication for CloudGraph server.
...
JWT_ALGORITHM = "HS256"
JWT_ISSUER = "cloudgraph"
JWT_AUDIENCE = "cloudgraph-api"
...
class AuthConfig:
    ...
    def __init__(
        ...
        self.jwt_secret = jwt_secret or os.environ.get("CLOUDGRAPH_JWT_SECRET", secrets.token_hex(32))
    ...
    def from_env(cls) -> "AuthConfig":
        ...
        enabled = os.environ.get("CLOUDGRAPH_AUTH_ENABLED", "true").lower() == "true"

        # Load API keys from environment (comma-separated name:hash pairs)
        api_keys = {}
        api_keys_env = os.environ.get("CLOUDGRAPH_API_KEYS", "")
        if api_keys_env:
        ...
        return cls(
            enabled=enabled,
            api_keys=api_keys,
            jwt_secret=os.environ.get("CLOUDGRAPH_JWT_SECRET"),
            allow_anonymous_health=os.environ.get("CLOUDGRAPH_ALLOW_ANON_HEALTH", "true").lower() == "true",
            allow_anonymous_read=os.environ.get("CLOUDGRAPH_ALLOW_ANON_READ", "false").lower() == "true",
        )
...
def require_auth(f: Optional[Callable] = None, *, allow_read: bool = False):
    ...
    Args:
        allow_read: If True, allows anonymous access when CLOUDGRAPH_ALLOW_ANON_READ is set
