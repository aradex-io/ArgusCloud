"""CloudGraph CLI - Multi-cloud security graph analytics.
...
def parse_args(args: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        prog="cloudgraph",
        description="CloudGraph - Multi-cloud security graph analytics",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Collect AWS data
  cloudgraph collect --provider aws --profile myprofile

  # Normalize collected data
  cloudgraph normalize --input ./cloudgraph-output

  # Run security rules
  cloudgraph analyze --input ./cloudgraph-output

  # Export findings
  cloudgraph export --format sarif --output findings.sarif

  # Start API server
  cloudgraph serve --port 5000
        """
    )
    ...
    collect_parser.add_argument(
        "--output", "-o",
        default="cloudgraph-output",
        help="Output directory for collected data (default: cloudgraph-output)"
    )
    collect_parser.add_argument(
        "--profile",
...
def cmd_analyze(args: argparse.Namespace) -> int:
    ...
    from awshound.bundle import write_jsonl
    from cloudgraph.core.graph import Node, Edge
    from cloudgraph.core.base import RuleContext
    from cloudgraph.rules import evaluate_all_rules
    ...
    if not nodes_path.exists() or not edges_path.exists():
        print("Normalized data not found. Run 'cloudgraph normalize' first.")
        return 1
    ...
    logger.info(
        f"Analysis: {len(new_findings)} from cloudgraph.rules, "
...
def cmd_export(args: argparse.Namespace) -> int:
    ...
    from cloudgraph.core.graph import GraphData, Node, Edge
    from cloudgraph.exporters import JSONExporter, SARIFExporter, HTMLExporter
...
def cmd_serve(args: argparse.Namespace) -> int:
    ...
    from cloudgraph.api import create_app
    ...
    print(f"CloudGraph API starting on http://{args.host}:{args.port}")
...
def cmd_keygen(args: argparse.Namespace) -> int:
    ...
    from cloudgraph.api.auth import generate_api_key
    ...
    print(f'  export CLOUDGRAPH_API_KEYS="mykey:{hashed_key}"')
    print()
...
def main(args: Optional[List[str]] = None) -> int:
    """Main entry point for CloudGraph CLI."""
    parsed = parse_args(args)

    if not parsed.command:
        print("Usage: cloudgraph <command> [options]")
        print("Run 'cloudgraph --help' for available commands")
        return 1
