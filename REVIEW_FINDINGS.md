# CloudGraph — Application Review Findings
...
**Evidence:** `COPY cloudhound/ ./cloudhound/`, `COPY cloudhound.py ./`, `CMD ["gunicorn", ..., "cloudhound.api.wsgi:application"]`. The package was renamed to `cloudgraph/` and `cloudhound.py` is in `.gitignore`.
...
**Recommendation:** Replace all `cloudhound/` and `cloudhound.py` references with `cloudgraph/` and update the gunicorn target to `cloudgraph.api.wsgi:application`.

### C-03 — Cypher injection via interpolated relationship type in `save_profile` / repository
**Location:** `cloudgraph/repositories/neo4j_repository.py:443-448`
**Evidence:**
```python
...
**Location:** `cloudgraph/api/wsgi.py:41`, `cloudgraph/api/server.py:990`, `cloudgraph/cli/main.py:181, 209`, `start.sh:24-27`, `.env.example:18`, `docker-compose.yml:63`
**Evidence:**
```python
neo4j_password = os.environ.get("CLOUDGRAPH_NEO4J_PASSWORD", "letmein123")
parser.add_argument("--password", default=os.environ.get("NEO4J_PASSWORD", "letmein123"))
```
```yaml
JWT_SECRET: ${JWT_SECRET:-dev-secret-change-in-production}
```
**Impact:** Any deployment that does not explicitly set `CLOUDGRAPH_NEO4J_PASSWORD` and `CLOUDGRAPH_JWT_SECRET` runs with publicly known credentials. The fallback JWT secret allows token forgery against any user. The Neo4j fallback exposes the full graph database to anyone reachable on port 7687.
**Recommendation:** Remove every literal `letmein123` and `dev-secret-change-in-production`. Fail-fast at startup if the env vars are unset (mirror the `${VAR:?error}` pattern already used in `docker-compose.prod.yml`). Centralize on `cloudgraph/config.py` (which correctly defaults to empty string).

### C-05 — Authentication disabled by default in dev compose and `start.sh`
**Location:** `docker-compose.yml:59`, `start.sh:25`
**Evidence:** `CLOUDGRAPH_AUTH_ENABLED: ${AUTH_ENABLED:-false}` and `DEFAULT_AUTH_ENABLED="false"`.
...
**Location:** `cloudgraph/api/server.py:create_app()` (and `wsgi.py`); `cloudgraph/api/metrics.py`, `cloudgraph/api/shutdown.py`, `cloudgraph/api/ratelimit.py`, `cloudgraph/api/errors.py`
...
### C-07 — Entire new rules engine (`cloudgraph/rules/aws/`) is dead code; only legacy `awshound` rules execute
**Location:** `cloudgraph/cli/main.py:328-351`
**Evidence:**
```python
...
```
The 23 modular rules under `cloudgraph/rules/aws/` (better-structured, with remediation, severity enums, IMDSv2-aware checks) are registered via decorators but never invoked. `cloudgraph.rules.evaluate_all_rules` has no call sites.
...
**Recommendation:** Switch `cmd_analyze` to call `cloudgraph.rules.evaluate_all_rules` (or call both engines and union the results, deduplicating). Add an end-to-end integration test asserting the new engine fires on a known fixture.
...
**Location:** `ui/js/app.js:1742-1746, 2856-2867, 3713-3726, 3717-3718, 3896-3900, 4231, 4477-4479`; `cloudgraph/exporters/html.py:183-191`
...
**Location:** `cloudgraph/api/server.py:129, 144-155`
**Evidence:**
```python
...
**Location:** `pyproject.toml` `[project.urls]`, `Dockerfile` OCI labels, `.env.example` `CLOUDGRAPH_VERSION`
...
**Recommendation:** Update URLs to `cloudgraph`. Derive version from a single source (`importlib.metadata.version("cloudgraph")`) and add a CI assertion that all version strings agree before release.
...
**Location:** `cloudgraph/api/server.py:495-577`, `cloudgraph/repositories/neo4j_repository.py:393-465`
...
**Location:** `cloudgraph/repositories/neo4j_repository.py` (entire), `awshound/storage.py:50-58`
...
### H-10 — Two divergent normalization paths (`awshound/normalize.py` vs `cloudgraph/normalizers/aws/`) produce different node schemas
**Location:** `awshound/normalize.py:197-209` vs `cloudgraph/normalizers/aws/iam.py:86-99` (and many more)
**Evidence:** Same resource, different keys: `role_name` vs `name`, `user_name` vs `name`. Date fields are raw `datetime` in awshound, stringified in cloudgraph. `_extract_principals` and `_is_admin_policy` are duplicated verbatim across both codebases.
...
**Location:** `cloudgraph/normalizers/aws/ec2.py:34`, `awshound/normalize.py:552-572`
...
**Location:** `awshound/rules.py:99-101`, `cloudgraph/normalizers/aws/ec2.py:216-218`
...
**Location:** `cloudgraph/api/models.py`, all POST handlers in `cloudgraph/api/server.py`
...
**Location:** `cloudgraph/api/server.py:1002`, `cloudgraph/cli/main.py:459`
...
**Impact:** Operators following the README (`cloudgraph serve --port 5000`) get a development server in production with poor concurrency, no graceful shutdown integration, and no signal handling consistent with gunicorn workers.
**Recommendation:** Make `cmd_serve` exec gunicorn (`subprocess.run(["gunicorn", "cloudgraph.api.wsgi:application", ...])`) or refuse to start when `CLOUDGRAPH_ENV=production`. At minimum print a loud warning.

### H-16 — `errors.py` infrastructure not registered: full Flask tracebacks leak to clients
**Location:** `cloudgraph/api/errors.py:85-147`, `cloudgraph/api/server.py:create_app()`
...
**Location:** `cloudgraph/api/collect.py:237, 263, 274`; `cloudgraph/api/server.py:587, 709`
...
**Location:** `cloudgraph/api/openapi.py:652-1230`, `cloudgraph/api/server.py` (no route serves the spec)
...
**Recommendation:** Add `.github/workflows/ci.yml` running `ruff check`, `mypy`, `pytest --cov=cloudgraph --cov-fail-under=60`, plus a Python matrix (3.9-3.12) and an integration job that exercises the testcontainers path.
...
**Location:** `cloudgraph/api/auth.py`, `cloudgraph/api/wsgi.py:41`, `docker-compose.yml:64`
...
**Location:** `cloudgraph/api/ratelimit.py:89, 98-105`
**Evidence:** `storage_uri = os.environ.get("CLOUDGRAPH_RATELIMIT_STORAGE", "memory://")`. Default gunicorn config is 4 workers; each has its own counter, so a documented "100 req/min" effectively becomes 400.
...
**Location:** `cloudgraph/api/server.py:686, 706`; `cloudgraph/collectors/session.py:86`
...
**Location:** `cloudgraph/api/server.py:815-820`
...
**Location:** `cloudgraph/api/collect.py:148-156`, `cloudgraph/api/uploads.py:110-118`
...
**Location:** `cloudgraph/api/collect.py:209-224`
...
**Location:** `cloudgraph/repositories/neo4j_repository.py:314-325`
...
**Location:** `cloudgraph/core/base.py:179-200, 203-223`
...
**Location:** `awshound/rules.py:124` vs `cloudgraph/rules/aws/logging.py:16`
...
**Location:** `awshound/rules.py:286` vs `cloudgraph/rules/aws/ec2.py:88`
...
**Location:** `awshound/rules.py:224-245`, `cloudgraph/rules/aws/iam.py:83-95`
...
**Location:** `cloudgraph/exporters/html.py:183-191`
...
### M-22 — `tests/test_rules_rds.py` imports legacy `awshound.rules`; doesn't cover the new `cloudgraph/rules/aws/data.py`
**Location:** `tests/test_rules_rds.py:1-8`
**Evidence:** 8-line file, one test, uses legacy module. The 5 functions in `cloudgraph/rules/aws/data.py` are uncovered there.
...
### M-24 — `cloudgraph/collectors/aws/` has 11 untested service collectors
...
**Recommendation:** `addopts = "-v --tb=short --cov=cloudgraph --cov=awshound --cov-report=term-missing --cov-fail-under=60"`. Increase floor over time.
...
**Location:** `cloudgraph/plugins/registry.py:18`, `cloudgraph/core/registry.py:148-151`
...
### M-30 — Dual codebase: `cloudgraph/collectors/aws/` is unused dead weight (or `awshound/` is)
**Location:** `cloudgraph/api/collect.py:176-177`, `cloudgraph/cli/main.py:250-291`
**Evidence:** Both API and CLI shell out to `awshound.collector`, `awshound.normalize`, `awshound.bundle`. `cloudgraph/collectors/aws/` (~2K LoC) has no live call path.
...
**Location:** `cloudgraph/api/auth.py:62-69, 84-89`
**Recommendation:** Add `iss="cloudgraph"`, `aud="cloudgraph-api"` at issuance and require them at decode.

### L-02 — `/plugins` endpoint discloses load errors unauthenticated
**Location:** `cloudgraph/api/server.py:191-200`
**Recommendation:** Require auth, or strip `errors` from anonymous responses.

### L-03 — CLI commands lack top-level exception handling
**Location:** `cloudgraph/cli/main.py:229-546`
...
**Location:** `cloudgraph/api/collect.py:106-124`
...
**Location:** `cloudgraph/core/graph.py:27-83`
...
**Location:** `cloudgraph/core/base.py:32, 147`
...
**Location:** `cloudgraph/exporters/sarif.py:127-133`
...
**Location:** `cloudgraph/collectors/session.py:47-52`
...
**Location:** `cloudgraph/repositories/neo4j_repository.py:62, 96`
...
**Location:** `cloudgraph/api/server.py:405-451, 589-612`
...
**Evidence:** `all = ["cloudgraph[dev,gcp,azure]"]` — `prod` (gunicorn, flask-limiter, prometheus-client) is missing.
...
**Location:** `cloudgraph/collectors/gcp/__init__.py`, `cloudgraph/collectors/azure/__init__.py`, `README.md:9, 57-58`, `pyproject.toml` extras
